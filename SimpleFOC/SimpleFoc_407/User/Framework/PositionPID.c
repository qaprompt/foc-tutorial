...
static void PositionPIDControl(PPositionPIDControl_Struct pPositionPID)
{
	...
	//旋转坐标系
	pPositionPID->pPID.pre -= pPositionPID->pPID.tar;
	if(pPositionPID->pPID.pre > 180) {
		pPositionPID->pPID.pre -= 360;
	} else if (pPositionPID->pPID.pre < -180) {
		pPositionPID->pPID.pre += 360;
	}
	
    pPositionPID->pPID.bais = 0 - pPositionPID->pPID.pre;
